...
import { DashboardIcon, UsersIcon, CameraIcon, UserCircleIcon, QuestionMarkCircleIcon, BookOpenIcon, ClipboardListIcon, CurrencyDollarIcon } from './IconComponents';
...
interface FarmerNavProps {
    ...
    onNavigate: (view: 'dashboard' | 'community' | 'scanner' | 'profile' | 'agronomist' | 'learningHub' | 'buyerRequestsFeed' | 'finance') => void;
...
const NavButton: React.FC<{
    ...
    <button
        onClick={onClick}
        className={`flex-1 flex flex-col items-center justify-center gap-1 py-2 px-2 rounded-lg transition-colors text-xs sm:text-sm font-medium ${
            isActive
...
export const FarmerNav: React.FC<FarmerNavProps> = ({ activeView, onNavigate, profile }) => {
    const mainViews: FarmerViewMode[] = ['dashboard', 'community', 'agronomist', 'scanner', 'learningHub', 'profile', 'buyerRequestsFeed', 'finance'];
    ...
    return (
        <div className="bg-white p-4 rounded-2xl shadow-lg border border-slate-200">
            ...
            <div className="flex flex-wrap items-center justify-around gap-1 sm:gap-2 bg-slate-100/70 p-1 rounded-xl">
                 <NavButton
                    label="Dashboard"
